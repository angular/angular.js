...
module.exports = {
  ...
  getVersionInfo: function() {
    if (grunt.config.get('NG_VERSION') && grunt.config.get('DIST_NAME')) {
      return true;
    }

    // Get the previous versions only once for current / cdn versions
    var previousVersions = versionInfo.getPreviousVersions();
    var NG_VERSION = versionInfo.getCurrentVersion(previousVersions);
    NG_VERSION.cdn = versionInfo.getCdnVersion(previousVersions);

    var DIST_NAME = 'angular-'+ NG_VERSION.full;

    grunt.config.set('NG_VERSION', NG_VERSION);
    grunt.config.set('DIST_NAME', DIST_NAME);
  }
};
