...
function main(argv) {
  ...
  compressor.writeErrorJson(cmd_line_arguments[2]);
/**  for (var key in compressor.errors_table) {
    var obj = compressor.errors_table[key];
    console.log(obj.err_code + ' : ' + obj.message + ' : ' + obj.min_message + ' : ' + obj.prod_message + '\n');
  }*/
}
...
Compressor.prototype.writeErrorJson = function(file_name) {
  fs.writeFile(file_name, JSON.stringify(this.errors_table, null, '\t'), function(err) {
    if (err) console.log(err);
  });
};
