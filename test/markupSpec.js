...
describe("markups", function(){
  ...
  it('should translate {{}} in terminal nodes', function(){
    ...
    scope.$digest();
    expect(sortedHtml(element).replace(' selected="true"', '')).toEqual('<select ng:model="x"><option ng:bind-template="Greet {{name}}!">Greet !</option></select>');
    scope.name = 'Misko';
    scope.$digest();
    expect(sortedHtml(element).replace(' selected="true"', '')).toEqual('<select ng:model="x"><option ng:bind-template="Greet {{name}}!">Greet Misko!</option></select>');
  });
