describe('api', function(){
  ...
  describe('limit', function() {
    ...
    it('should return the first X items when X is positive', function() {
      expect(angular.Array.limit(items, 3)).toEqual(['a', 'b', 'c']);
      expect(angular.Array.limit(items, '3')).toEqual(['a', 'b', 'c']);
    });


    it('should return the last X items when X is negative', function() {
      expect(angular.Array.limit(items, -3)).toEqual(['f', 'g', 'h']);
      expect(angular.Array.limit(items, '-3')).toEqual(['f', 'g', 'h']);
    });


    it('should return an empty array when X cannot be parsed', function() {
      expect(angular.Array.limit(items, 'bogus')).toEqual([]);
      expect(angular.Array.limit(items, 'null')).toEqual([]);
      expect(angular.Array.limit(items, 'undefined')).toEqual([]);
      expect(angular.Array.limit(items, null)).toEqual([]);
      expect(angular.Array.limit(items, undefined)).toEqual([]);
    })
  });
