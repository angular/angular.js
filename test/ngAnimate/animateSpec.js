...
describe("animations", function() {
  ...
  describe('$animate', function() {
    ...
    it('should fire only ONE reflow right after the digest occurs to flush all animation preparation styles',
      inject(function($animate, $rootScope, $document, $rootElement, $window) {

      $animate.enabled(true);

      var oldChild = jqLite('<div></div>');
      var element = jqLite('<div></div>');
      var parent = jqLite('<div></div>');
      ...
      var blockSpy = spyOn($window, 'forcePageReflow').andCallThrough();

      $animate.leave(oldChild);
      $animate.enter(element, parent);
      $animate.addClass(parent, 'gold');
      ...
      expect(capturedAnimationHistory.length).toBe(3);
    }));
