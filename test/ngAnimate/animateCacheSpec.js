...
describe("ngAnimate $$animateCache", function() {
  ...
  it('should store whether the stored entry is invalid or not', inject(function($$animateCache) {
    ...
    expect($$animateCache.containsCachedValidAnimation('key1')).toBe(true);
    expect($$animateCache.containsCachedValidAnimation('key2')).toBe(false);
  }));

  it('should consider a cached entry valid if it does not exist in the cache', inject(function($$animateCache) {
    expect($$animateCache.containsCachedValidAnimation('key2')).toBe(true);
