...
describe('ngRepeatStart', function() {
  ...
  it('should iterate over an array of objects', function() {
    ...
    // INIT
    scope.items = [{name: 'misko', number: 1, color: 'red'}, {name:'shyam', number: 2, color: 'blue'}];
    scope.$digest();
    expect(element.find('li').length).toEqual(6);
    expect(element.text()).toEqual('misko;1;red;shyam;2;blue;');
    delete Array.prototype.extraProperty;

    // GROW
    scope.items.push({name: 'adam', number: 3, color: 'green'});
    scope.$digest();
    expect(element.find('li').length).toEqual(9);
    expect(element.text()).toEqual('misko;1;red;shyam;2;blue;adam;3;green;');
    ...
    expect(element.text()).toEqual('shyam;2;blue;');
  });
  ...
  it('should iterate over on object/map', function() {
    element = $compile(
      '<ul>' +
        '<li ng-repeat-start ng-repeat="(key, value) in items">{{key}}:</li>' +
  ...
  describe('track by', function() {
    it('should track using expression function', function() {
      element = $compile(
          '<ul>' +
            '<li ng-repeat-start ng-repeat="item in items track by item.id">{{item.id}};</li>' +
            '<li ng-repeat-end>{{item.id}};</li>' +
          '</ul>')(scope);
      scope.items = [{id: 'misko'}, {id: 'igor'}];
      scope.$digest();
      var li0 = element.find('li')[0];
      var li1 = element.find('li')[1];
      var li2 = element.find('li')[2];
      var li3 = element.find('li')[3];

      scope.items.push(scope.items.shift());
      scope.$digest();
      expect(element.find('li')[0]).toBe(li2);
      expect(element.find('li')[1]).toBe(li3);
      expect(element.find('li')[2]).toBe(li0);
      expect(element.find('li')[3]).toBe(li1);
    });
    ...
    it('should track using build in $id function', function() {
      element = $compile(
          '<ul>' +
            '<li ng-repeat-start ng-repeat="item in items track by $id(item)">{{item.name}};</li>' +
    ...
    it('should iterate over an array of primitives', function() {
      element = $compile(
          '<ul>' +
            '<li ng-repeat-start ng-repeat="item in items track by $index">{{item}};</li>' +
            '<li ng-repeat-end>{{item}};</li>' +
          '</ul>')(scope);
      ...
      scope.$digest();
      expect(element.find('li').length).toEqual(6);
      expect(element.text()).toEqual('true;true;true;true;true;true;');
      delete Array.prototype.extraProperty;

      scope.items = [false, true, true];
      scope.$digest();
      expect(element.find('li').length).toEqual(6);
      expect(element.text()).toEqual('false;false;true;true;true;true;');

      scope.items = [false, true, false];
      scope.$digest();
      expect(element.find('li').length).toEqual(6);
      expect(element.text()).toEqual('false;false;true;true;false;false;');

      scope.items = [true];
      scope.$digest();
      expect(element.find('li').length).toEqual(2);
      expect(element.text()).toEqual('true;true;');

      scope.items = [true, true, false];
      scope.$digest();
      expect(element.find('li').length).toEqual(6);
      expect(element.text()).toEqual('true;true;true;true;false;false;');

      scope.items = [true, false, false];
      scope.$digest();
      expect(element.find('li').length).toEqual(6);
      expect(element.text()).toEqual('true;true;false;false;false;false;');

      // string
      scope.items = ['a', 'a', 'a'];
      scope.$digest();
      expect(element.find('li').length).toEqual(6);
      expect(element.text()).toEqual('a;a;a;a;a;a;');

      scope.items = ['ab', 'a', 'a'];
      scope.$digest();
      expect(element.find('li').length).toEqual(6);
      expect(element.text()).toEqual('ab;ab;a;a;a;a;');

      scope.items = ['test'];
      scope.$digest();
      expect(element.find('li').length).toEqual(2);
      expect(element.text()).toEqual('test;test;');

      scope.items = ['same', 'value'];
      scope.$digest();
      expect(element.find('li').length).toEqual(4);
      expect(element.text()).toEqual('same;same;value;value;');

      // number
      scope.items = [12, 12, 12];
      scope.$digest();
      expect(element.find('li').length).toEqual(6);
      expect(element.text()).toEqual('12;12;12;12;12;12;');

      scope.items = [53, 12, 27];
      scope.$digest();
      expect(element.find('li').length).toEqual(6);
      expect(element.text()).toEqual('53;53;12;12;27;27;');

      scope.items = [89];
      scope.$digest();
      expect(element.find('li').length).toEqual(2);
      expect(element.text()).toEqual('89;89;');

      scope.items = [89, 42];
      scope.$digest();
      expect(element.find('li').length).toEqual(4);
      expect(element.text()).toEqual('89;89;42;42;');
    });
    ...
    it('should iterate over object with changing primitive property values', function() {
      ...
      element = $compile(
          '<ul>' +
            '<li ng-repeat-start ng-repeat="(key, value) in items track by $index">{{key}}:</li>' +
            '<li>{{value}};</li>' +
            '<li ng-repeat-end><input type="checkbox" ng-model="items[key]"></li>' +
          '</ul>')(scope);
  ...
  it('should not ngRepeatStart over parent properties', function() {
    var Class = function() {};
  ...
  it('should error on wrong parsing of ngRepeatStart', function() {
    element = jqLite('<ul><li ng-repeat-start ng-repeat="i dont parse"></li><li ng-repeat-end></li></ul>');
  ...
  it("should throw error when left-hand-side of ngRepeat can't be parsed", function() {
    element = jqLite('<ul><li ng-repeat-start ng-repeat="i dont parse in foo"></li><li ng-repeat-end></li></ul>');
    $compile(element)(scope);
    expect($exceptionHandler.errors.shift()[0].message).
  ...
  it('should error on unexpected ngRepeatEnd', function() {
    element = jqLite('<ul><li ng-repeat-end ng-repeat="i in items"></li><li></li></ul>');
    try {
      $compile(element)(scope);
      fail();
    } catch(e) {
      expect(e.message).toBe('Unexpected ngRepeatEnd.');
    }
  });


  it('should error on unmatched ngRepeatStart', function() {
    element = jqLite('<ul><li ng-repeat-start ng-repeat="i in items"></li><li></li></ul>');
    try {
      $compile(element)(scope);
      fail();
    } catch(e) {
      expect(e.message).toBe('Unmatched ngRepeatStart.');
    }
  });
  ...
  it('should expose iterator offset as $index when iterating over arrays',
    ...
    element = $compile(
      '<ul>' +
        '<li ng-repeat-start ng-repeat="item in items">{{item}}:</li>' +
        '<li ng-repeat-end>{{$index}}|</li>' +
      '</ul>')(scope);
  ...
  it('should expose iterator offset as $index when iterating over objects', function() {
    element = $compile(
      '<ul>' +
        '<li ng-repeat-start ng-repeat="(key, val) in items">{{key}}:</li>' +
        '<li>{{val}}:</li>' +
        '<li ng-repeat-end>{{$index}}|</li>' +
      '</ul>')(scope);
  ...
  it('should expose iterator position as $first, $middle and $last when iterating over objects',
    ...
    element = $compile(
      '<ul>' +
        '<li ng-repeat-start ng-repeat="(key, val) in items">{{key}}:{{val}}:</li>' +
        '<li ng-repeat-end>{{$first}}-{{$middle}}-{{$last}}|</li>' +
      '</ul>')(scope);
  ...
  it('should ignore $ and $$ properties', function() {
    element = $compile('<ul><li ng-repeat-start ng-repeat="i in items">{{i}}|</li><li ng-repeat-end>{{i}}</li></ul>')(scope);
    scope.items = ['a', 'b', 'c'];
    scope.items.$$hashKey = 'xxx';
    scope.items.$root = 'yyy';
    scope.$digest();

    expect(element.text()).toEqual('a|ab|bc|c');
  });
  ...
  it('should iterate over non-existent elements of a sparse array', function() {
    element = $compile('<ul><li ng-repeat-start ng-repeat="item in array track by $index">{{item}}</li><li ng-repeat-end>|</li></ul>')(scope);
    scope.array = ['a', 'b'];
    scope.array[4] = 'c';
    scope.array[6] = 'd';
    scope.$digest();
  ...
  describe('stability', function() {
    ...
    beforeEach(function() {
      element = $compile(
        '<ul>' +
          '<li ng-repeat-start ng-repeat="item in items">{{key}}</li><li>:{{val}}</li><li ng-repeat-end>|</li>' +
        '</ul>')(scope);
      a = {};
      b = {};
      c = {};
    ...
    it('should preserve the order of elements', function() {
      ...
      expect(newElements[3]).toEqual(lis[6]);
      expect(newElements[6]).not.toEqual(lis[2]);
    });
    ...
    it('should reverse items when the collection is reversed', function() {
      ...
      scope.$digest();
      var newElements = element.find('li');
      expect(newElements.length).toEqual(9);
      expect(newElements[0]).toEqual(lis[6]);
      expect(newElements[3]).toEqual(lis[3]);
      expect(newElements[6]).toEqual(lis[0]);
    });
    ...
    it('should reuse elements even when model is composed of primitives', function() {
      ...
      expect(newLis.length).toEqual(9);
      expect(newLis[0]).toEqual(lis[6]);
      expect(newLis[3]).toEqual(lis[0]);
      expect(newLis[6]).toEqual(lis[3]);
    });
  });
});

describe('ngRepeatStart ngAnimate', function() {
  ...
  it('should fire off the enter animation + add and remove the css classes',
    ...
    element = $compile(html(
      '<div>' +
        '<div ng-repeat-start ng-repeat="item in items" ng-animate="{enter: \'custom-enter\'}">{{ item }}</div>' +
        '<div ng-repeat-end>{{item}}</div>' +
      '</div>'
    ))($rootScope);
  ...
  it('should fire off the leave animation + add and remove the css classes',
    ...
    element = $compile(html(
      '<div>' +
        '<div ng-repeat-start ng-repeat="item in items" ng-animate="{leave: \'custom-leave\'}">{{ item }}</div>' +
        '<div ng-repeat-end>{{item}}</div>' +
      '</div>'
    ))($rootScope);
  ...
  it('should fire off the move animation + add and remove the css classes',
    inject(function($compile, $rootScope, $sniffer) {
      element = $compile(html(
        '<div>' +
          '<div ng-repeat-start ng-repeat="item in items" ng-animate="{move: \'custom-move\'}">{{ item }}</div>' +
          '<div ng-repeat-end>{{ item }}</div>' +
        '</div>'
      ))($rootScope);
      ...
      var left  = jqLite(kids[2]);
      var right = jqLite(kids[4]);
