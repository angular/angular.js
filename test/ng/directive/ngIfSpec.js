...
describe('ngIf', function () {
  ...
  it('should restore the element to its compiled state', function() {
    ...
    makeIf('value');
    expect(element.children().length).toBe(1);
    $(element.children()[0]).removeClass('my-class');
...
describe('ngIf ngAnimate', function () {
  ...
  it('should fire off the enter animation + add and remove the css classes',
    inject(function ($compile, $rootScope, $sniffer) {
      ...
      if ($sniffer.supportsTransitions) {
        expect(element.children()[0].className).toContain('custom-enter-setup');
        window.setTimeout.expect(1).process();
        expect(element.children()[0].className).toContain('custom-enter-start');
        window.setTimeout.expect(1000).process();
      } else {
        expect(window.setTimeout.queue).toEqual([]);
      }

      expect(element.children()[0].className).not.toContain('custom-enter-setup');
      expect(element.children()[0].className).not.toContain('custom-enter-start');
  }));
  ...
  it('should fire off the leave animation + add and remove the css classes',
    inject(function ($compile, $rootScope, $sniffer) {
      ...
      if ($sniffer.supportsTransitions) {
        expect(element.children()[0].className).toContain('custom-leave-setup');
        window.setTimeout.expect(1).process();
        expect(element.children()[0].className).toContain('custom-leave-start');
        window.setTimeout.expect(1000).process();
      } else {
        expect(window.setTimeout.queue).toEqual([]);
      }
