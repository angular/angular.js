...
describe('ngRepeat', function() {
  ...
  it('should iterate over an array of objects', function() {
    ...
    scope.$digest();
    expect(element.find('li').length).toEqual(3);
    expect(element.text()).toEqual('shyam;adam;misko;');
    ...
    expect(element.text()).toEqual('adam;');
  });
...
describe('ngRepeatInner', function() {
  var element, $compile, scope, $exceptionHandler;
  ...
  it('should iterate over an array of objects', function() {
    element = $compile(
      '<table>' +
        '<tr ng-repeat-inner="item in items"><td>{{item.name}};</td><td>{{item.color}};</td></tr>' +
      '</table>')(scope);

    Array.prototype.extraProperty = "should be ignored";
    // INIT
    scope.items = [{name: 'misko', color:'red'}, {name:'shyam', color:'blue'}];
    scope.$digest();
    expect(element.find('tr').length).toEqual(1);
    expect(element.find('tr').find('td').length).toEqual(4);
    expect(element.text()).toEqual('misko;red;shyam;blue;');
    delete Array.prototype.extraProperty;

    // GROW
    scope.items.push({name: 'adam', color:'green'});
    scope.$digest();
    expect(element.find('tr').find('td').length).toEqual(6);
    expect(element.text()).toEqual('misko;red;shyam;blue;adam;green;');

    // SWAP
    scope.items.push(scope.items.shift());
    scope.$digest();
    expect(element.find('tr').find('td').length).toEqual(6);
    expect(element.text()).toEqual('shyam;blue;adam;green;misko;red;');

    // SHRINK
    scope.items.pop();
    scope.items.shift();
    scope.$digest();
    expect(element.find('tr').find('td').length).toEqual(2);
    expect(element.text()).toEqual('adam;green;');

    // No elements still keep the parent element
    scope.items.pop();
    scope.$digest();
    expect(element.find('tr').length).toEqual(1);
    expect(element.find('tr').find('td').length).toEqual(0);
  });
  ...
  it('should iterate over on object/map', function() {
    element = $compile(
      '<ul>' +
        '<li ng-repeat-inner="(key, value) in items">{{key}}:{{value}}|</li>' +
      '</ul>')(scope);
    scope.items = {misko:'swe', shyam:'set'};
    scope.$digest();
    expect(element.text()).toEqual('misko:swe|shyam:set|');
  ...
  describe('track by', function() {
    it('should track using expression function', function() {
      element = $compile(
          '<ul ng-repeat-inner="item in items track by item.id">' +
              '<li>{{item.name}};</li>' +
          '</ul>')(scope);
      scope.items = [{id: 'misko'}, {id: 'igor'}];
      scope.$digest();
    ...
    it('should track using build in $id function', function() {
      element = $compile(
          '<ul ng-repeat-inner="item in items track by $id(item)">' +
              '<li>{{item.name}};</li>' +
          '</ul>')(scope);
      scope.items = [{name: 'misko'}, {name: 'igor'}];
      scope.$digest();
    ...
    it('should iterate over an array of primitives', function() {
      element = $compile(
          '<ul ng-repeat-inner="item in items track by $index">' +
              '<li>{{item}};</li>' +
          '</ul>')(scope);
    ...
    it('should iterate over object with changing primitive property values', function() {
      ...
      element = $compile(
          '<ul ng-repeat-inner="(key, value) in items track by $index">' +
            '<li>' +
              '{{key}}:{{value}};' +
              '<input type="checkbox" ng-model="items[key]">' +
            '</li>' +
  ...
  it('should not ngRepeatInner over parent properties', function() {
    var Class = function() {};
    Class.prototype.abc = function() {};
    Class.prototype.value = 'abc';

    element = $compile(
      '<ul ng-repeat-inner="(key, value) in items">' +
        '<li>{{key}}:{{value}};</li>' +
      '</ul>')(scope);
    scope.items = new Class();
    scope.items.name = 'value';
  ...
  it('should error on wrong parsing of ngRepeatInner', function() {
    element = jqLite('<ul ng-repeat-inner="i dont parse"><li></li></ul>');
    $compile(element)(scope);
    expect($exceptionHandler.errors.shift()[0].message).
        toBe("Expected ngRepeatInner in form of '_item_ in _collection_[ track by _id_]' but got 'i dont parse'.");
  });


  it("should throw error when left-hand-side of ngRepeatInner can't be parsed", function() {
      element = jqLite('<ul ng-repeat-inner="i dont parse in foo"><li></li></ul>');
      $compile(element)(scope);
    expect($exceptionHandler.errors.shift()[0].message).
        toBe("'item' in 'item in collection' should be identifier or (key, value) but got 'i dont parse'.");
  });
  ...
  it('should expose iterator offset as $index when iterating over arrays',
      function() {
    element = $compile(
      '<ul ng-repeat-inner="item in items">' +
        '<li>{{item}}:{{$index}}|</li>' +
      '</ul>')(scope);
    scope.items = ['misko', 'shyam', 'frodo'];
    scope.$digest();
  ...
  it('should expose iterator offset as $index when iterating over objects', function() {
    element = $compile(
      '<ul ng-repeat-inner="(key, val) in items">' +
        '<li>{{key}}:{{val}}:{{$index}}|</li>' +
      '</ul>')(scope);
    scope.items = {'misko':'m', 'shyam':'s', 'frodo':'f'};
    scope.$digest();
  ...
  it('should expose iterator position as $first, $middle and $last when iterating over objects',
    ...
    element = $compile(
      '<ul ng-repeat-inner="(key, val) in items">' +
        '<li>{{key}}:{{val}}:{{$first}}-{{$middle}}-{{$last}}|</li>' +
      '</ul>')(scope);
    scope.items = {'misko':'m', 'shyam':'s', 'doug':'d', 'frodo':'f'};
    scope.$digest();
    expect(element.text()).
  ...
  it('should ignore $ and $$ properties', function() {
    element = $compile('<ul ng-repeat-inner="i in items"><li>{{i}}|</li></ul>')(scope);
    scope.items = ['a', 'b', 'c'];
    scope.items.$$hashKey = 'xxx';
    scope.items.$root = 'yyy';
    scope.$digest();
  ...
  it('should repeat over nested arrays', function() {
    element = $compile(
      '<div><ul ng-repeat-inner="subgroup in groups">' +
        '<li ng-repeat-inner="group in subgroup">' +
          '{{group}}|' +
        '</li>X' +
      '</ul></div>')(scope);
    scope.groups = [['a', 'b'], ['c','d']];
  ...
  it('should ignore non-array element properties when iterating over an array', function() {
    element = $compile('<ul ng-repeat-inner="item in array"><li>{{item}}|</li></ul>')(scope);
    scope.array = ['a', 'b', 'c'];
    scope.array.foo = '23';
    scope.array.bar = function() {};
    scope.$digest();
  ...
  it('should iterate over non-existent elements of a sparse array', function() {
    element = $compile('<ul ng-repeat-inner="item in array track by $index"><li>{{item}}|</li></ul>')(scope);
    scope.array = ['a', 'b'];
    scope.array[4] = 'c';
    scope.array[6] = 'd';
    scope.$digest();
...
describe('ngRepeatInner ngAnimate', function() {
  var vendorPrefix, window;
  var body, element;
  ...
  it('should fire off the enter animation + add and remove the css classes',
    ...
    element = $compile(html(
      '<div ng-repeat-inner="item in items" ng-animate="{enter: \'custom-enter\'}"><div>' +
        '{{ item }}' + 
      '</div></div>'
    ))($rootScope);
  ...
  it('should fire off the leave animation + add and remove the css classes',
    ...
    element = $compile(html(
      '<div ng-repeat-inner="item in items" ng-animate="{leave: \'custom-leave\'}"><div>' +
        '{{ item }}' + 
      '</div></div>'
    ))($rootScope);
  ...
  it('should fire off the move animation + add and remove the css classes',
    inject(function($compile, $rootScope, $sniffer) {
      element = $compile(html(
        '<div ng-repeat-inner="item in items" ng-animate="{move: \'custom-move\'}">' +
          '<div>' +
            '{{ item }}' +
          '</div>' +
        '</div>'
