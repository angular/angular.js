...
angularWidget('@ng:repeat', function(expression, element){
  ...
  return function(iterStartElement){
    ...
    // item from the iterator, and the value is an array of objects with following
    // properties.
    //   - scope: bound scope
    //   - element: previous element.
    //   - index: position
    // We need an array of these objects since the same object can be returned
    // from the iterator. We expect this to be a rare case.
        ...
        if (collection.hasOwnProperty(key)) {
          ...
          if (last) {
            ...
            if (index === last.index) {
              ...
              last.index = index;
              // This may be a noop, if the element is next,
              // but I don't know of a good way to figure this out,
              // since it would require extra DOM access, so
              // let's just hope that the browsers realizes that
              // it is noop, and treats it as such.
