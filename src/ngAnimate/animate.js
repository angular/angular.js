 ...
 * and the module must be included as a dependency.
 *
 * <pre>
 ...
 * through by using the `$animate` service.
 *
...
angular.module('ngAnimate', ['ng'])
  ...
  .config(['$provide', '$animateProvider', function($provide, $animateProvider) {
    ...
    /**
     * @ngdoc function
     * @name ngAnimate.$animate#register
     * @methodOf ngAnimate.$animateProvider
     *
     * @description
     ...
     */
    $animateProvider.register = function(name, factory) {
    ...
    $provide.decorator('$animate', ['$delegate', '$injector', '$window', '$sniffer', '$rootElement',
      ...
      $rootElement.data(NG_ANIMATE_CONTROLLER, rootAnimateController);
       ...
       function lookup(name) {
        if (name) {
          ...
          angular.forEach(classes, function(n) {
            classMap[n] = true;
          });

          var matches = [];
          angular.forEach(selectors, function(selectorFactory, index) {
            var found = true;
            for(var i=0;i<selectorFactory.selectors.length;i++) {
              var klass = selectorFactory.selectors[i];
              if(klass.length > 0) {
      ...
      */
      function performAnimation(event, className, element, parent, after, onComplete) {
        if(!isPrimeForAnimation(className, element)) {
          (onComplete || angular.noop)();
        } else {
          var classes = ((element.attr('class') || '') + ' ' + className),
              animationLookup = (' ' + classes).replace(/\s+/g,'.'),
              animations = [];
          angular.forEach(lookup(animationLookup), function(animation, index) {
            animations.push({
          ...
          if ((parent.inheritedData(NG_ANIMATE_CONTROLLER) || disabledAnimation).running) {
          ...
          angular.forEach(animations, function(animation, index) {
            var fn = function() {
              progress(index);
            };
        ...
        function suffixClasses(classes, suffix) {
          ...
          angular.forEach(classes, function(klass, i) {
  ...
  .animation('', ['$window','$sniffer', function($window, $sniffer) {
    ...
    function animate(element, className, done) {
      if (!angular.isFunction($window.getComputedStyle) || (!$sniffer.transitions && !$sniffer.animations)) {
        done();
      } else {
        var activeClassName = '';
      ...
      function startAnimation() {
        var duration = 0;
        angular.forEach(className.split(' '), function(klass, i) {
