 ...
 *
 * @description
 * Use `$formFactory` to create a new scope instance of a {@link guide/dev_guide.forms form}
 * controller or to find the nearest form scope instance for a given DOM element.
 *
 * A form is a scope object with `$createWidget` method. As new widgets are created using
 * `$createWidget` their instances are assigned to the form. Having the widget published on the
 * form allows the UI to data-bind to the internal state of the widget, to display validation
 * error messages.
 *
        ...
        <script>
          ...
          angular.directive('ng:contenteditable', function(){
            function linkFn($formFactory, element) {
              ...
              widget = form.$createWidget(this, exp, exp, null, ContentEditableWidget, element);
...
angularServiceInject('$formFactory', function(){
   ...
   *
   * Each application gets a root form which is the ultimate parent of all forms.
   */
  formFactory.root = formFactory(this);
  ...
  formFactory.forElement = function (element) {
    return element.inheritedData('$form') || formFactory.root;
  };
  return formFactory;

  function formFactory(parent) {
    return (parent || formFactory.root).$new(FormController);
  }
 ...
 * @description
 * Widget is responsible for emitting this event whenever the view changes do to user interaction.
 ...
 * @description
 *
 * Instance method on a form used to create new widgets of this form.
 *
 ...
 *
 * @param {Scope} modelScope The scope where the model for the widget is stored.
 * @param {String} modelExp The angular assignable expression to bind to on the model scope.
 * @param {String} changeExp Expression to execute when user changes the value.
 * @param {String} name The name where the widget instance should be published on the form.
 * @param {WidgetController} WidgetController The controller constructor function.
 * @param {...} ... Any extra arguments will be curried to the WidgetController constructor.
 */
FormController.prototype.$createWidget = function(modelScope, modelExp, changeExp, name, WidgetController) {
  assertArg(modelExp, 'exp');
  var form = this,
      scopeGet = parser(modelExp).assignable(),
      scopeSet = scopeGet.assign,
      widget = this.$new(WidgetController, sliceArgs(arguments, 5));

  widget.$modelScope = modelScope;
  widget.$error = {};
  ...
  widget.$on('$viewChange', function(event, viewValue){
    if (!equals(widget.$viewValue, viewValue)) {
      ...
      if (changeExp) modelScope.$eval(changeExp);
      widget.$emit('$validate');
    }
  });
  ...
  if (name && !form.hasOwnProperty(name)) {
    form[name] = widget;
    widget.$widgetId = name;
  } else {
    name = null;
  }
