...
function $CompileProvider($provide) {
  ...
  this.$get = [
     ...
     * @param {function(angular.Scope[, cloneAttachFn]} transcludeFn
    ...
    function compileNodes(nodeList, transcludeFn, rootElement, maxPriority) {
     ...
     /* nodesetLinkigFn */ function linkingFn(scope, nodeList, rootElement, boundTranscludeFn) {
        ...
        if (newScopeDirective && isObject(newScopeDirective.scope)) {
          forEach(newScopeDirective.scope, function(mode, name) {
            (LOCAL_MODE[mode] || wrongMode)(name, mode,
                newScopeDirective ? scope.$parent || scope : scope, scope, attrs);
          });
        }
      ...
      $http.get(asyncWidgetDirective.templateUrl, {cache: $templateCache}).
        success(function(content) {
          ...
          while(linkQueue.length) {
            var boundTranscludeFn = linkQueue.pop(),
            ...
            afterWidgetLinkFn(function() {
              beforeWidgetLinkFn(afterWidgetChildrenLinkFn, scope, node, rootElement, boundTranscludeFn);
...
function directiveLinkingFn(
  ...
  /* Node */ node,
  /* Element */ rootElement,
  /* function(Function) */ eboundTranscludeFn
){}
