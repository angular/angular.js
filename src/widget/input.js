    ...
    annotate('$defer', '$formFactory', function($defer, $formFactory, inputElement){
      var form = $formFactory.forElement(inputElement),
          ...
          patternMatch = pattern
            ? pattern.match(/^\/(.*)\/$/) && (pattern = new RegExp(pattern.substring(1, pattern.length - 2)))
                ? function(value) { return pattern.test(value); }
                : function(value) { return (modelScope.$eval(pattern)||{test:noop}).test(value); }
            : valueFn(true),
