...
function composeResourceUrl(template, params, stripTrailingSlashes) {
  var protocolAndIpv6 = '',
  ...
  // stripTrailingSlashes defaults to true
  stripTrailingSlashes = (stripTrailingSlashes === undefined) || stripTrailingSlashes;
  ...
  return { url: url, params: queryParams };
}

var $composeResourceUrlFactory = ['$http', function($http) {
  var defaultParamSerializer = $http.defaults && $http.defaults.paramSerializer;
   ...
   * @description
   *
   * Compose a hash of parameters against a template to produce URL string.
   * The result will be similar to the URL used in `$resource` HTTP requests (with the caveat below).
   *
   * Parameters that match placeholders in the template will be inserted into the URL.
   * Any parameters that do not match placeholders in the template will be added to the URL query.
   *
   * The default `$http` `paramSerializer` will be used to serialize the extra query parameters unless
   * a specific serializer is given in the `options` argument.
   *
   * **Caveat**:
   * The `$resource` service delegates query parameters serialization to the `$http` request.
   * If there is a `$http` request interceptor configured that modifies the query parameters before
   * they are serialized then the URL used by `$resource` may not match that generated by this service.
   *
   ...
   * @param {Object} [options]  A hash of options that modify the compostion behaviour. The options available are:
   *    * `stripTrailingSlashes` (default: true) whether to remove slashes from the end of the URL.
   *    * `paramSerializer` (default: `$http.defaults.paramSerializer`) the serializer to use when serializing extra query parameters.
   *
   * @returns {string} The composed URL.
   */
  return function $composeResourceUrl(template, params, options) {
    var config = composeResourceUrl(template, params, options && options.stripTrailingSlashes);
    var queryParamString = (options && options.paramSerializer || defaultParamSerializer)(config.params);
    if (queryParamString) {
      queryParamString = (config.url.indexOf('?') !== -1 ? '&' : '?') + queryParamString;
    }
    return config.url + queryParamString;
  };
}];
    ...
    this.$get = ['$http', '$log', '$q', '$timeout', '$composeResourceUrl', function($http, $log, $q, $timeout, $composeResourceUrl) {
      ...
      Route.prototype = {
        setUrlParams: function(config, params, actionUrl) {
          extend(config, composeResourceUrl(actionUrl || this.template, params, this.options.stripTrailingSlashes));
        }
      };
