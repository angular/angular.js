        ...
        function animateActionFactory(type, beforeFn, afterFn) {
          return function(element, parent, after) {
            ...
            function parseMaxTime(str) {
              var total = 0, values = safelySplitByComma(str);
                ...
                forEach(element, function(element) {
                  if (element.nodeType == ELEMENT_NODE) {
                    var w3cProp = w3cTransitionProp,
                        ...
                        iterations = 1,
                        globalStyles = $window.getComputedStyle(element) || {};

                    //use CSS Animations over CSS Transitions
                    if(parseFloat(globalStyles[w3cAnimationProp + durationKey]) > 0 ||
                       parseFloat(globalStyles[vendorAnimationProp + durationKey]) > 0) {
                      w3cProp = w3cAnimationProp;
                      vendorProp = vendorAnimationProp;
                      iterations = Math.max(parseInt(globalStyles[w3cProp    + animationIterationCountKey]) || 0,
                                            parseInt(globalStyles[vendorProp + animationIterationCountKey]) || 0,
                                            iterations);
                    }

                    var parsedDelay     = Math.max(parseMaxTime(globalStyles[w3cProp     + delayKey]),
                                                   parseMaxTime(globalStyles[vendorProp  + delayKey]));

                    var parsedDuration  = Math.max(parseMaxTime(globalStyles[w3cProp     + durationKey]),
                                                   parseMaxTime(globalStyles[vendorProp  + durationKey]));
